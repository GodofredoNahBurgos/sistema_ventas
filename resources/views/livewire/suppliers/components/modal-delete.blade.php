<flux:modal name="delete-supplier" class="min-w-[22rem]" :dismissible="false" >
    <div class="space-y-6">
        <div class="my-6">
        ...
        <div class="flex gap-2">
            <flux:spacer />
            <flux:modal.close>
                <flux:button class="cursor-pointer" icon="x-mark" variant="primary" wire:click="resetSelectedSupplier">Cancelar</flux:button>
            </flux:modal.close>
            <flux:button class="cursor-pointer" icon="trash" variant="danger"
                wire:click="delete({{ $selectedSupplierId }})" wire:loading.attr="disabled">Eliminar
            </flux:button>
        </div>
    </div>
</flux:modal>