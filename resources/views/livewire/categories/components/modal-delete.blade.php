<flux:modal name="delete-category" class="min-w-[22rem]" :dismissible="false">
    <div class="space-y-6">
        ...
        <div class="flex gap-2">
            ...
            <flux:modal.close>
                <flux:button class="cursor-pointer" icon="x-mark" variant="primary" wire:click="resetSelectedCategory">Cancelar</flux:button>
            </flux:modal.close>

            <flux:button class="cursor-pointer" icon="trash" variant="danger"
                wire:click="delete({{ $selectedCategoryId }})" wire:loading.attr="disabled">Eliminar
            </flux:button>
        </div>
    </div>
</flux:modal>